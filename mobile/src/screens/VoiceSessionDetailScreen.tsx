...
interface VoiceSessionDetailScreenProps {
  user: any;
  theme: ThemeColors;
  baseUrl: string;
  route: any;
...
export const VoiceSessionDetailScreen = ({ user, theme, baseUrl, route, navigation }: VoiceSessionDetailScreenProps) => {
  ...
  useEffect(() => {
    const fetchHistory = async () => {
      ...
      try {
        ...
        const msgRes = await axios.get(`${baseUrl}/api/chat/conversations/${initialConversationId}/messages`);
          if (msgRes.data.status === 'success') {
            ...
            if (history.length > 0) {
              ...
              setMessages([{ id: 'initial-1', sender: 'ai', content: `No messages found for this session.` }]);
            }
          }
      } catch (error) {
